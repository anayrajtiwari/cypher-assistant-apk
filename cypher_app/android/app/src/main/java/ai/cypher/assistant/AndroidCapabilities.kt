...
class AndroidCapabilities(
    ...
    fun getBatteryStatus(): String {
        ...
        return "Battery: $pct% ${if (charging) "CHARGING" else "DISCHARGING"}"
    }
    ...
    fun getStorageStatus(): String {
        ...
        return "Storage: ${total}GB total, ${free}GB free"
    }

    fun listInstalledApps(): String {
        val pm = context.packageManager
        val apps = pm.getInstalledApplications(PackageManager.GET_META_DATA)
        return apps.take(50).joinToString("\n") { "${it.loadLabel(pm)} (${it.packageName})" }
    }

    fun readContacts(): String {
        if (!permissions.checkExplicit("read_contacts")) return "Permission denied."
        ...
        return contacts.take(50).joinToString("\n").ifEmpty { "No contacts found." }
    }

    fun getLocation(): String {
        if (!permissions.checkExplicit("get_location")) return "Permission denied."
        try {
    ...
    fun sendSms(number: String, message: String): String {
        if (!permissions.checkExplicit("send_sms")) return "Permission denied."
        return try {
    ...
    fun makePhoneCall(number: String): String {
        if (!permissions.checkExplicit("make_phone_call")) return "Permission denied."
    ...
    fun takePhoto(): String {
        if (!permissions.checkExplicit("take_photo")) return "Permission denied."
        ...
        context.startActivity(intent)
        return "Camera opened. Photo saved to ${file.absolutePath}."
    }
    ...
    fun setVolume(stream: String = "music", level: Int = 10): String {
        ...
        return "Volume set to $safeLevel/$max for $stream."
    }
    ...
    fun clipboardRead(): String {
        ...
        return if (clip != null && clip.itemCount > 0) clip.getItemAt(0).text?.toString() ?: "Empty"
