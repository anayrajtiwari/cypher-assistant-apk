    ...
    class WebAppInterface(private val context: Context) {
        ...
        fun downloadModel(url: String) {
            val appDir = context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)
                ?: context.filesDir
            val destFile = java.io.File(appDir, "cypher-1.5b-q4_0.gguf")
            ...
            try {
                ...
                (context as AppCompatActivity).runOnUiThread {
                    Toast.makeText(context, "Model download started to app storage!", Toast.LENGTH_LONG).show()
                }
            } catch (e: Exception) {
                try {
                    val request = DownloadManager.Request(Uri.parse(url)).apply {
                    ...
                    (context as AppCompatActivity).runOnUiThread {
                        Toast.makeText(context, "Public Download started...", Toast.LENGTH_LONG).show()
                    }
                } catch (e2: Exception) {
                    (context as AppCompatActivity).runOnUiThread {
                        Toast.makeText(context, "Download failed: ${e2.message}", Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
